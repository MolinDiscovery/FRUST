...
| Compare aligned structures by RMSD | `compare_rmsd` |
...
```mermaid
flowchart LR
    A["FRUST dataframe"] --> B["plot_mols, plot_vibs, plot_conformers"]
    C["Molecules, XYZ files, reactions"] --> D["MolTo3DGrid or RxnTo3DGrid"]
    E["Relative energies"] --> F["plot_energy_profile"]
```

Normal notebooks can use the direct helpers:
